...
export default {
  ...
  computed: {
    department () {
      return this.$store.getters['departments/getById'](Number.parseInt(this.$route.params.id))
    },
  ...
  async mounted () {
    try {
      ...
      await Promise.all([
        ...
        this.$store.dispatch('costCenters/fetch')
      ])

      this.data.name = this.department.name
      this.data.costCenterId = this.department.costCenterId
    } finally {
      this.loading = false
    }
  },
  ...
  methods: {
    async update () {
      try {
        ...
        await this.$store.dispatch('departments/update', {
