...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        ...
        password: [
          v => (v ? v.length >= 3 && v.length <= 72 : true) || 'Senha deve conter de 3 a 72 caracteres'
        ],
        position: [
  ...
  head () {
    return {
      title: this.$t('section.users.update')
    }
  },

  computed: {
    user () {
      return this.$store.getters['users/getById'](Number.parseInt(this.$route.params.id))
    },
  ...
  async mounted () {
    try {
      ...
      this.data.positionId = this.user.positionId
      this.data.departmentId = this.user.departmentId
    } finally {
      this.loading = false
    }
  },
